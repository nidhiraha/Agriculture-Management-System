...
const Produce = () => {
    const [values, setValues] = useState({
        ...
        added_quantity: "",
        amount: "",
        percentage_remaining: ""  // New state for remaining percentage
    });

    const navigate = useNavigate();

    // Effect to calculate the percentage remaining whenever relevant fields change
    useEffect(() => {
    ...
    const handleSubmit = (e) => {
        ...
        axios.post('http://localhost:3000/auth/inventory', values)
            .then(res => {
                console.log(res);
